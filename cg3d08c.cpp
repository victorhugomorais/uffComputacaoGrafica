...
int main(){
	...
	TriLines<Elements<Triangles>> TL{indices};

	int w = 800, h = 600;
	Graphics3D G{w, h};
	mat4 View = lookAt({4, 4 ,1}, {0, 0, -1}, {0, 0, 1});
	float a = w/(float)h;
	mat4 Projection = perspective(40, a, 0.1, 50);
	...
	for(int k = 0; k < nframes; ++k){
		...
		color_shader.M = Projection*View*Model;
		G.draw(V, T, color_shader);

		SimpleShader simple_shader;
		simple_shader.M = translate(0, 0, -0.0001)*color_shader.M;
		simple_shader.C = black;

		G.draw(V, TL, simple_shader);
